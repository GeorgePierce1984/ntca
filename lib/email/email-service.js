...
async function getTeacherNotificationEmail(teacher) {
  if (teacher?.user?.email || teacher?.email) {
    return teacher.user?.email || teacher.email;
  }
  ...
  try {
    ...
    console.error("Failed to resolve teacher email:", error);
    return null;
  }
}
...
export const emailHelpers = {
  ...
  async notifyTeacherOfStatusUpdate(teacher, job, school, newStatus, note = null) {
    const toEmail = await getTeacherNotificationEmail(teacher);

    if (!toEmail) {
      return { success: false, skipped: true, reason: "missing_recipient" };
    }

    return await sendEmail('applicationStatusUpdate', toEmail, {
